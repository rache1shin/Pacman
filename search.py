...
def uniformCostSearch(problem):
    ...
    while not pQueue.isEmpty():
        ...
        for s in successors:    # [ (x,y), 'Direction', 1 ]
            tmpState = s[0]
            if tmpState not in visited:
                tmpCost = s[2]+cost
                tmpPath = list(path)
                tmpPath.append(s[1])
                pQueue.update([tmpState, tmpCost, tmpPath], tmpCost)
                if not problem.isGoalState(tmpState):
